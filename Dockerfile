...
RUN go get github.com/golang/dep/cmd/dep && \
    ...
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a --installsuffix cgo --ldflags="-s -w"  -ldflags "-X main.version=$(git log | head -n 1 | cut  -f 2 -d ' ') -X main.build_date=$(date +%Y-%m-%d\-%H:%M)" -o /kuberbs
RUN strip /kuberbs
